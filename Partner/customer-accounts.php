...
if (isset($_GET['idPerson'])) {
    $idPartner = $_GET['idPerson'];
    $sql = "SELECT p.`Names`, p.`Surnames`, p.`DUI`, p.`Trade`, p.`Location`, p.`PhoneNumber`,p.`Status`, u.`Mail`, u.`Password`
        FROM partners p
            ...
            <div class="animated fadeIn">
                <div class="row">
                    ...
                    $sql = "SELECT a.*,CONCAT(c.`Names`, ' ', c.`Surnames`) AS FullName
                            FROM accounts a
                            INNER JOIN partners c ON c.`UserAccount` = a.`Customer`
                            WHERE a.`Customer` = '" . $idPartner . "'";
                    ...
                    if(mysqli_num_rows($resultado) >= 0 && mysqli_num_rows($resultado) <=2){
                        echo '<div class="col-md-4">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <div class="mx-auto d-block">
                                            <h5 class="text-sm-center mt-2 mb-1"><a href="accounts-creator.php?idPerson='.$idPartner.'"><i class="fa-solid fa-circle-plus text-sm-center" style="font-size: 18rem;"></i></a></h5>
                                        </div>
                                    </div>
                                </div>
                            </div>';
...
<?php
    if (isset($_GET['error'])) {
        echo "<script>Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
                            text: 'Usted posee el maximo de cuentas'})
                </script>";
    }
    ...
    if (isset($_GET['success'])) {
        echo "<script>Swal.fire({
                            icon: 'success',
                            title: 'Success',
                            text: 'Se ha solicitado su nueva cuenta'})
                </script>";
    }
    ...
    if (isset($_GET['withdrawal'])) {
        echo "<script>Swal.fire({
                            icon: 'success',
                            title: 'Success',
                            text: 'Se ha procesado la transferencia'})
                </script>";
    }
    ?>
