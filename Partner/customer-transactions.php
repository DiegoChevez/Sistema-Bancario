...
require_once('../Tools/connexion.php');
 if(isset($_GET["idCuenta"])){
    ...
    $sql = "SELECT t.*,u.`Username` FROM transactions t INNER JOIN users u ON u.`ID_User` = t.Author;";
    ...
    <div id="right-panel" class="right-panel">
        ...
        <div class="content">
            ...
            <div class="header-content" style="display: flex; align-items: center; gap: 10px;">
                ...
                <h3>Movimientos de la cuenta: <?php echo $idCuenta ?> </h3>
            </div>
                            ...
                            <div class="card-body">
                                <table id="bootstrap-data-table" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        $sql = "SELECT t.*, u.`Username` FROM transactions t INNER JOIN users u ON u.`ID_User` = t.Author WHERE t.`RootAccount` = '".$idCuenta."' OR t.`DestinationAccount` = '".$idCuenta."' OR (t.`RootAccount` = '".$idCuenta."' AND t.`DestinationAccount` = '".$idCuenta."')";                                
...
<?php
    ...
    if(isset($_GET["bug1"])){
        echo "<script>Swal.fire({
                ...
                text: 'No se ha podido realizar las tranferencias'})
            </script>";
    }
    if(isset($_GET["bug2"])){
        echo "<script>Swal.fire({
                icon: 'error',
                title: 'Ha ocurrido un error',
                text: 'No se han encontrado la cuenta'})
            </script>";
    }
